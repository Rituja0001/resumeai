...
This guide details how to deploy the **ResumeAI Backend** (Django + PostgreSQL + Redis + Celery) to [Render](https://render.com). The frontend is hosted separately on Vercel.

---

## Architecture Overview

1. **Web Service (`resumeai-backend`)**: Django API running with Gunicorn and WhiteNoise for static files.
2. **Background Worker (`resumeai-worker`)**: Celery worker executing background AI tasks.
3. **Managed Database (`resumeai-db`)**: Render PostgreSQL database.
4. **Managed Redis (`resumeai-redis`)**: Render Redis instance acting as Celery broker & result backend.
5. **Frontend (Vercel)**: Communicates with the Render backend via REST API (`VITE_API_BASE`).

---

## Option 1: One-Click Deploy via Render Blueprint (`render.yaml`) (Recommended)

Render Blueprints allow provisioning all 4 resources automatically from `render.yaml`.

### Steps:
1. **Push your repository to GitHub** (make sure `render.yaml`, `backend/requirements.txt`, and `backend/core/settings.py` changes are included).
...
5. Render will detect `render.yaml` and display the resources to be created:
   - Web Service: `resumeai-backend`
   - Worker Service: `resumeai-worker`
   - Database: `resumeai-db`
   - Redis: `resumeai-redis`
...
7. Click **Apply**. Render will provision and launch all resources in sequence.
...
- **Name**: `resumeai-redis`
- **Plan**: Free (or Starter)
- **IP Allow List**: Leave empty for internal access only.
- Click **Create Redis**. Copy the **Internal Redis URL** once created.
...
| `REDIS_URL` | Auto-populated by Render from `resumeai-redis` (`fromService`) |

---

### Variables to Set Manually in Render Dashboard:

> **Important**: Set these for **BOTH** the Web Service (`resumeai-backend`) and Background Worker (`resumeai-worker`) unless noted otherwise.

| Variable | Target Service | Example / Recommended Value | Description |
|---|---|---|---|
| `DJANGO_SECRET_KEY` | Web & Worker | *Generate a strong secret key* | Django encryption key (auto-generated in Blueprint). |
| `DJANGO_DEBUG` | Web & Worker | `False` | Must be `False` in production. |
...
| `ANTHROPIC_API_KEY` | Web & Worker | `sk-ant-api03-...` | Claude API key for resume generation & analysis. |
| `ANTHROPIC_MODEL` | Web & Worker | `claude-sonnet-4-6` | AI model identifier. |
...
| `AWS_STORAGE_BUCKET_NAME` | Web & Worker | *(Optional)* | S3 bucket name if using AWS S3 for media storage. |
| `AWS_ACCESS_KEY_ID` | Web & Worker | *(Optional)* | AWS IAM access key ID. |
| `AWS_SECRET_ACCESS_KEY` | Web & Worker | *(Optional)* | AWS IAM secret access key. |
| `AWS_S3_REGION_NAME` | Web & Worker | `ap-south-1` | AWS S3 region. |
...
This ensures database migrations execute automatically and safely on each deploy before the app starts handling web traffic.
