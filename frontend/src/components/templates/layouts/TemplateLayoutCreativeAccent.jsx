    ...
    <div className="w-full h-full bg-white p-2.5 sm:p-3 flex flex-col justify-start select-none font-['Plus_Jakarta_Sans'] text-[#252525] overflow-hidden text-[5.5px]">
      <div className="space-y-1.5 overflow-hidden flex-1 flex flex-col justify-between">
        ...
        <div className="bg-slate-50 border border-slate-200/80 p-1.5 rounded-lg flex justify-between items-center">
          <div>
            <h4 className="font-mono font-extrabold text-[10px] text-[#252525] leading-tight">
              {sampleName}
            </h4>
          ...
          <div className="text-right text-[4.8px] font-mono text-[#666]">
            <span className="bg-blue-50 text-blue-700 px-1 py-0.2 rounded border border-blue-200 block mb-0.5 truncate">
              {sampleLinks && sampleLinks.length > 0 ? sampleLinks[0].url.replace(/^https?:\/\//, '') : sampleGithub}
            </span>
        ...
        {sampleExperience.length > 0 && (
          <div>
            <span className="font-mono font-bold text-[6px] uppercase tracking-wider block mb-0.5" style={{ color: accentColor }}>
              // Work Experience
            </span>
              ...
              {sampleProjects.slice(0, 2).map((proj, idx) => (
                <div key={idx} className="text-[5.2px]">
                  <div className="flex justify-between items-baseline">
                    ...
                    <span className="text-[#888] font-mono text-[4.8px] shrink-0">{proj.techStack || proj.tech_stack}</span>
                  </div>
                  {proj.description && (
        ...
        {/* Education & Info */}
        <div className="border-t border-slate-100 pt-1 space-y-0.8 font-mono">
          <div className="flex justify-between text-[5.2px] text-[#555]">
            <span className="truncate">// Edu: {sampleEducation[0]?.degree} · {sampleEducation[0]?.institution}</span>
          ...
          {(sampleLanguages.length > 0 || sampleHobbies) && (
            <div className="flex justify-between text-[4.8px] text-[#777]">
              {sampleLanguages.length > 0 && (
                <span className="truncate">Lang: {sampleLanguages.slice(0, 3).map((l) => typeof l === "string" ? l : l.name).join(", ")}</span>
              )}
              {sampleHobbies && (
